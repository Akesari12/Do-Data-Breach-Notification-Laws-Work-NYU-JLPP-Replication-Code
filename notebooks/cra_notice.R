...
consumer_cra_notice <- consumer_sentinel %>%
...
consumer_cra_notice <- consumer_cra_notice %>%
  filter(!state %in% c('Rhode Island', 'Missouri', 'Michigan', 'Maryland')) %>%
  ...
  mutate(effective_year = ifelse(is.na(effective_year),
                                 Inf, effective_year)) %>%
  mutate(cra_notice_required = 1 * (year >= effective_year)) %>%
...
ppool_syn <- multisynth(id_theft_per_capita ~ cra_notice_required | 
                          percent_broadband +  
...
base_id_theft_rate <- consumer_cra_notice %>%
  filter(state %in% c(ppool_syn_summ$att$Level)) %>%
  filter(cra_notice_required == 1) %>%
...
cra_notice_bar_effects <- ppool_syn_summ$att %>%
  filter(Time > 0) %>%
  group_by(Level) %>%
  ...
  scale_fill_manual(values=c("green",
                             ...
                             'red')) +
  theme_classic() +
  ggtitle('Average Treatment Effects by State for Consumer Credit Reporting Agency Notification',
...
cra_notice_plot <- ppool_syn_summ$att %>%
  ...
  ggtitle('Staggered Synthetic Controls for Required Notification to Credit Reporting Agency') +
  xlab('Time') +
...
ggsave(filename = here('images', 'cra_notice.png'),
       plot = cra_notice_plot,
       device = 'png',
       scale = 1,
       width = 4512,
       height = 2072,
...
ggsave(filename = here('images', 'cra_notice_bar_effects.png'),
       plot = cra_notice_bar_effects,
       device = 'png',
       scale = 1,
       width = 4512,
       height = 2072,
