...
consumer_ag_notice <- consumer_ag_notice %>%
  filter(!state %in% c('Florida', 'Arizona'))
...
outlier_ag_notice_bar_effects <- ppool_syn_summ$att %>%
  filter(Time > 0) %>%
  group_by(Level) %>%
  ...
  scale_fill_manual(values=c("green",
                             ...
                             'red')) +
  theme_classic() +
  ggtitle('ATTs w/ Outliers Removed for State Regulator Notification',
...
outlier_ag_notice_plot <- ppool_syn_summ$att %>%
  filter(Time >= -5) %>%
  ...
  ggtitle('Trends w/ Outliers Removed \nfor Notification to Regulators') +
...
source(here('notebooks', 'cra_notice.R'))

consumer_cra_notice <- consumer_cra_notice %>%
  filter(!state %in% c('Georgia', 'New Mexico'))
...
outlier_cra_notice_bar_effects <- ppool_syn_summ$att %>%
  filter(Time > 0) %>%
  group_by(Level) %>%
  ...
  scale_fill_manual(values=c("green",
                             ...
                             'red')) +
  theme_classic() +
  ggtitle('ATTs w/ Outliers Removed for Consumer Credit Reporting Agency Notification',
          subtitle = paste0('Average ATT ', 
                            round(percent_reduction, 2),
                            '%')) +
  xlab('State') +
...
outlier_cra_notice_plot <- ppool_syn_summ$att %>%
  filter(Time >= -5) %>%
  filter(!Level %in% c('New Mexico', 'Nebraska', 'Oregon', 'South Dakota', 
                       'Florida', 'Arizona', 'Alabama', 'Hawaii', 'Georgia',
  ...
  xlab('Time Before and After Enactment of Notification to CRAs') +
  ylab('Identity Theft Per 100K') +
  theme(legend.position = "none") +
  #facet_wrap(~Level) +
  theme(strip.text.x = element_text(size = 15)) + 
